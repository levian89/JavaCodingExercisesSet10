package com.myprohects.JavaCodingExercisesSet10;

import java.util.Scanner;

public enum Direction {
    NORTH("North"),
    SOUTH("South"),
    EAST("East"),
    WEST("West");

    private String name;

    Direction(String name) {
        this.name = name;
    }

    public String getName() {
        return name;
    }

    public Direction left() {
        // You are to implement the logic that correctly gets the Direction on the "left"
        // If we represent the directions in a circular format going clockwise
        // then the direction to the "left" of NORTH is WEST, of WEST is SOUTH, of SOUTH is EAST, and of EAST is NORTH
        switch (this) {
            case NORTH:
                return WEST;
            case SOUTH:
                return EAST;
            case EAST:
                return NORTH;
            case WEST:
                return SOUTH;
        }
        return this;
    }

    public Direction right() {
        // Similarly, you are to implement the logic that correctly gets the Direction on the "right"
        // If we represent the directions in a circular format going clockwise
        // then the direction to the "right" of NORTH is EAST, of EAST is SOUTH, of SOUTH is WEST, and of WEST is NORTH
        switch (this) {
            case NORTH:
                return EAST;
            case SOUTH:
                return WEST;
            case EAST:
                return SOUTH;
            case WEST:
                return NORTH;
        }
        return this;
    }

    private static Direction getEnumFromUser() {
        Scanner scanner = new Scanner(System.in);
        System.out.println("Enter the enum: ");
        String stringProvidedByUser = scanner.nextLine();
        Direction enumProvidedByUser = Direction.valueOf(stringProvidedByUser); // transforming the string provided by
                                                                                // user into an enum of Direction type
//        assertTrue(enumProvidedByUser == Direction.NORTH);
        return enumProvidedByUser;
    }

    public static void main(String[] args) {
        System.out.println("Left direction from the enum " + NORTH + " is: " + Direction.NORTH.left());

        Direction enumFromUser = getEnumFromUser();
        System.out.println("Right direction from the enum " + enumFromUser + " provided by user is: " + enumFromUser.right());
    }
}